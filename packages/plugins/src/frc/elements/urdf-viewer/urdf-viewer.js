...
export const elementConfig = {
  ...
  properties: {
    ...
    ambientColor: { type: String, defaultValue: 'black', attribute: 'ambient-color' },
    minDistance: { type: Number, defaultValue: .25, attribute: 'min-distance' },
    ...
    autoRotateSpeed: { type: Number, defaultValue: 2, attribute: 'auto-rotate-speed' },
  },
  slots: [
    { name: '', allowedChildren: ['frc-urdf-cylinder'] },
  ],
  demos: [{
    // html: `
    //   <frc-urdf-viewer
...
class UrdfViewer extends LitElement {
  ...
  render() {
    return html`   
      <urdf-viewer part="viewer" urdf-content="${this.#urdfContent}" @urdf-error="${this.onUrdfError}" up="${this.up}"
        ?display-shadow="${this.displayShadow}" ambient-color="${this.ambientColor}" .loadMeshFunc="${loadMesh}"
        min-distance="${this.minDistance}" max-distance="${this.maxDistance}" robot-x="${this.robotX}"
        robot-y="${this.robotY}" robot-z="${this.robotZ}" camera-x="${this.cameraX}" camera-y="${this.cameraY}"
        camera-z="${this.cameraZ}" ?auto-rotate="${this.autoRotate}" auto-rotate-speed="${this.autoRotateSpeed}"
        @camera-change="${this.onCameraChange}"></urdf-viewer>
    `;
  }
}
