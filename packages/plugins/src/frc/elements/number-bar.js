...
export default class NumberBar extends LitElement {
  ...
  render() {
    ...
    return html`
      <frc-bar value="${this.value}" min="${min}" max="${max}" center="${this.center}" part="bar">
        ${!this.hideText ? html`
        ${this.value.toFixed(clamp(this.precision, 0, 100))}
        ${this.unit}
        ` : ''}
      </frc-bar>
      ${this.numTickMarks > 0 ? html`
      <frc-table-axis part="axis" ticks="${this.numTickMarks}" .range="${[min, max]}" unit="${this.unit}"></frc-table-axis>
      ` : ''}
    `;
  }
}
